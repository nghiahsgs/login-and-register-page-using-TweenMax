var email = document.querySelector('.loginName'),
...
function getCoord(e) {
    var control = email;
    if (e != undefined) {
        control = document.querySelector('.' + e.target.className);
    }
    var carPos = control.selectionEnd,
		div = document.createElement('div'),
		span = document.createElement('span'),
		copyStyle = getComputedStyle(control),
    ...
    div.textContent = control.value.substr(0, carPos);
    span.textContent = control.value.substr(carPos) || '.';
    div.appendChild(span);

    emailCoords = getPosition(control);							//console.log("emailCoords.x: " + emailCoords.x + ", emailCoords.y: " + emailCoords.y);
...
function onEmailInput(e) {
    ...
    if (curEmailIndex > 0) {
        ...
        if (e.target.className == 'email' && value.includes("@")) {
            mouthStatus = "large";
        ...
        else if (e.target.className == 'loginName' && /^\d+$/.test(value)) {
            mouthStatus = "large";
            TweenMax.to([mouthBG, mouthOutline, mouthMaskPath], 1, { morphSVG: mouthLargeBG, ease: Expo.easeOut });
            TweenMax.to(tooth, 1, { x: 3, y: -2, ease: Expo.easeOut });
            TweenMax.to(tongue, 1, { y: 2, ease: Expo.easeOut });
...
if (fullname != null) {
    fullname.addEventListener('focus', onEmailFocus);
    fullname.addEventListener('blur', onEmailBlur);
    fullname.addEventListener('input', onEmailInput);
}
